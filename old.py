...
# RHUBARB_BIN = (
#     os.path.join(BIN_DIR, "rhubarb.exe")
#     if platform.system() == "Windows"
#     else os.path.join(BIN_DIR, "rhubarb")
# )
...
# # ---------------------------------------------------------------------------
# SYSTEM_PROMPT = """
# You are Rey, a medical assistant.
# Return ONLY JSON:
# {
#   "messages": [
#     {
...
# # Animations
# # ---------------------------------------------------------------------------
# TALKING = ["TalkingOne", "TalkingTwo", "TalkingThree"]
# GESTURE = ["DismissingGesture", "ThoughtfulHeadShake"]
...
#         if n == 1:
#             anim = random.choice(TALKING)
#         elif i == 0:
#             anim = random.choice(TALKING)
#         elif i == n - 1:
#             anim = random.choice(TALKING)
#         else:
#             anim = random.choice(TALKING + GESTURE)
...
#     for i in range(1, n):
#         if messages[i]["animation"] == messages[i - 1]["animation"]:
#             messages[i]["animation"] = random.choice(
#                 [a for a in TALKING if a != messages[i]["animation"]]
#             )
...
#     msgs = data.get("messages", DEFAULT_RESPONSE)

#     return assign_natural_animation(msgs)
...
# def lip_sync(index):
#     mp3 = os.path.join(AUDIOS_DIR, f"message_{index}.mp3")
#     wav = os.path.join(AUDIOS_DIR, f"message_{index}.wav")
...
#         subprocess.run(
#             [RHUBARB_BIN, "-f", "json", "-o", json_path, wav, "-r", "phonetic"],
#             check=False,
#         )
#     else:
#         with open(json_path, "w") as f:
...
# # ---------------------------------------------------------------------------
# # SSE STREAM (FIXED ARCHITECTURE)
# # ---------------------------------------------------------------------------
# @app.route("/tts_stream", methods=["POST"])
# def tts_stream():
#     start = log_event("REQUEST")
...
#         msgs = chat_with_gemini(user_message)
...
#                 # TTS
#                 loop.run_until_complete(tts(msg["text"], mp3))

#                 # lip sync (blocking per message = stable animation sync)
#                 future = executor.submit(lip_sync, i)
#                 future.result()

#                 msg["audio"] = to_base64(mp3)
#                 msg["lipsync"] = read_json(
#                     os.path.join(AUDIOS_DIR, f"message_{i}.json")
#                 )

#                 msg["messageIndex"] = i
#                 msg["totalMessages"] = len(msgs)
...
# def tts_batch():
#     data = request.get_json()
#     msgs = chat_with_gemini(data.get("message", ""))
